...
float rayTrace(in vec3 origin,  in vec3 direction, in vec4 pos, out vec2 uv)
{
	...
	float dist = 32.0;
	float fdist = 32.0;
	float stc = objectScale.y / 0.5; // step count
	float mx = objectScale.y; // max y
	float sts = mx / stc; // step size;
	float cy;
	vec3 p = pos.xyz;
	
	for(float layer= 0.0; layer <= mx ; layer += sts)
	{
		...
		cy = texture2D(tex0, cuv).r * objectScale.y;
		error = abs((p.y - final.y) - cy);
		
		if(error > sts){dist = 32.0; continue;}
		if(final.x > objectScale.x ) {dist = 32.0; continue;}
		if(final.x < -objectScale.x) {dist = 32.0; continue;}
		if(final.z > objectScale.z ) {dist = 32.0; continue;}
		if(final.z < -objectScale.z) {dist = 32.0; continue;}
		
		if(error >= 32.0)
		{
			dist = length(final - or);
			fdist = dist;
			uv = cuv;
			minerror = error;
		}
