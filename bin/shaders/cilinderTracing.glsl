...
float projectToPlane(in vec3 p, vec3 norm , vec3 size, in vec3 or, vec3 dir)
{
	vec3 A = or + p.xyz;
	vec3 B = A + dir;
...
float map(vec3 position, out vec2 uv)
{
	//uv = position.xy * 0.5;
	//uv += vec2(0.5);
	uv.x = atan(position.x / position.z) / 6.283185307179586476925286766559;
...
float rayTrace(in vec3 origin,  in vec3 direction, in vec4 pos, out vec2 uv)
{
	...
	vec3 size = vec3(objectScale);
	vec3 p = pos.xyz;
	p.z = pos.z - objectScale.z;
	float front = projectToPlane(p, norm, size, or, dir);
	p.z = pos.z + objectScale.z;
	float back = projectToPlane(p, norm, size, or, dir);
	p = pos.xyz;
	p.x = pos.x - objectScale.x;
	norm = vec3(1.0, 0.0, 0.0);
	float left = projectToPlane(p, norm, size, or, dir);
	p.x = pos.x + objectScale.x;
	float right = projectToPlane(p, norm, size, or, dir);
	p = pos.xyz;
	p.y = pos.y - objectScale.y;
	norm = vec3(0.0, 1.0, 0.0);
	float bottom = projectToPlane(p, norm, size, or, dir);
	p.y = pos.y + objectScale.y;
	float top = projectToPlane(p, norm, size, or, dir);
	
	float stepper = min(min(min(front, back), min(left, right)), min(top, bottom));
	...
	for(int i=0; i<64; i++)
	{
		...
		distance = map( tposition.xyz / objectScale, uv);
...
void main() 
{
	...
	//gl_FragColor = vec4(vol, DecodeFloatRGBA(vec4(1.0)), 0.0,0.0);
}